...
+ (nullable NSArray <OCHTTPRequest *> *)detectionRequestsForConnection:(OCConnection *)connection options:(nullable OCAuthenticationMethodDetectionOptions)options; //!< Provides a list of URLs whose content is needed to determine whether this authentication method is supported
+ (void)detectAuthenticationMethodSupportForConnection:(OCConnection *)connection withServerResponses:(NSDictionary<NSURL *, OCHTTPRequest *> *)serverResponses options:(OCAuthenticationMethodDetectionOptions)options completionHandler:(void(^)(OCAuthenticationMethodIdentifier identifier, BOOL supported))completionHandler; //!< Detects authentication method support using collected responses (for URL provided by -detectionRequestsForConnection:options:) and then returns result via the completionHandler.
...
extern OCAuthenticationMethodKey OCAuthenticationMethodWebFingerAccountLookupURLKey; //!< If a WebFinger lookup provided the base URL for auth detection, this contains the URL from which an authenticated request can lookup available servers and other information.
extern OCAuthenticationMethodKey OCAuthenticationMethodWebFingerAlternativeIDPKey; //!< If a WebFinger lookup provided the base URL for auth detection, that URL is provided here.
