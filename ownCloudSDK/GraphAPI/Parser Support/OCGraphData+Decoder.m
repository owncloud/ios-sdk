...
@implementation NSDictionary (OCGraphDataDecoder)
...
+ (nullable id)object:(id)inObject key:(NSString *)key ofClass:(Class)class inCollection:(nullable Class)collectionClass required:(BOOL)required context:(nullable OCGraphContext *)context error:(NSError * _Nullable * _Nullable)outError
{
	...
	if (object != nil)
	{
		...
		else if ([object isKindOfClass:NSDictionary.class] && [class conformsToProtocol:@protocol(OCGraphObject)])
		{
			// Try parsing as OCGraphObject
			id decodedObject;

			if ((decodedObject = [((Class<OCGraphObject>)class) decodeGraphData:object context:nil error:outError]) != nil)
			{
