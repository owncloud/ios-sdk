...
- (OCCoreSyncInstruction)handleResultWithContext:(OCSyncContext *)syncContext
{
	...
	__block OCItem *resultItem = self.archivedServerItem;
	NSError *downloadError = event.error;

	if ((event.error == nil) && (event.file != nil) && (resultItem != nil))
	{
		NSError *error = nil;
		NSURL *vaultItemURL = [self.core.vault localURLForItem:resultItem];
		NSString *vaultItemLocalRelativePath = [self.core.vault relativePathForItem:resultItem];
		...
		if (useDownloadedFile)
		{
			if ((latestVersionOfItem = [self.core retrieveLatestVersionAtPathOfItem:resultItem withError:NULL]) != nil)
			{
				...
				if (latestVersionOfItem.locallyModified)
				{
					...
					issue = [OCSyncIssue issueForSyncRecord:syncRecord level:OCIssueLevelError title:OCLocalized(@"File modified locally") description:[NSString stringWithFormat:OCLocalized(@"\"%@\" was modified locally before the download completed."), resultItem.name] metaData:nil choices:@[
						// Drop sync record
						[OCSyncIssueChoice cancelChoiceWithImpact:OCSyncIssueChoiceImpactNonDestructive],
					]];
		...
		if (useDownloadedFile)
		{
			...
			BOOL vaultItemParentExists = [[NSFileManager defaultManager] fileExistsAtPath:vaultItemParentURL.path];

			if (!vaultItemParentExists)
			{
			...
			if (error == nil)
			{
				__block NSError *returnError = error;

				void(^storeFile)(NSURL *vaultItemURL, BOOL moveExistingToTempoary) = ^(NSURL *vaultItemURL, BOOL moveExistingToTempoary) {
					NSError *error = returnError;
					NSURL *existingFileTemporaryURL = nil;

					if (vaultItemParentExists && moveExistingToTempoary)
					{
					...
					if (error == nil)
					{
						...
						if ([[NSFileManager defaultManager] moveItemAtURL:event.file.url toURL:vaultItemURL error:&error])
						{
							...
							if (![resultItem.eTag isEqual:event.file.eTag])
							{
								if ((![resultItem.remoteItem.eTag isEqual:event.file.eTag]) && (resultItem.remoteItem != nil))
								{
									[resultItem.remoteItem prepareToReplace:resultItem];
									resultItem = resultItem.remoteItem;
								}
								else if (![latestVersionOfItem.eTag isEqual:event.file.eTag] && (latestVersionOfItem != nil))
								{
									[latestVersionOfItem prepareToReplace:resultItem];
									resultItem = latestVersionOfItem;
								}
							}

							resultItem.localRelativePath = vaultItemLocalRelativePath;
							resultItem.localCopyVersionIdentifier = [[OCItemVersionIdentifier alloc] initWithFileID:event.file.fileID eTag:event.file.eTag];

							resultItem.downloadTriggerIdentifier = self.options[OCCoreOptionDownloadTriggerID];
							resultItem.fileClaim = [OCClaim combining:self.localItem.fileClaim with:self.options[OCCoreOptionAddFileClaim] usingOperator:OCClaimGroupOperatorOR];
							...
							if (claimPurpose != OCCoreClaimPurposeNone)
							{
								...
								if ((temporaryClaim = [self.core generateTemporaryClaimForPurpose:claimPurpose]) != nil)
								{
									resultItem.fileClaim = [OCClaim combining:resultItem.fileClaim with:temporaryClaim usingOperator:OCClaimGroupOperatorOR];
									event.file.claim = temporaryClaim;

									[self.core removeClaim:temporaryClaim onItem:resultItem afterDeallocationOf:@[event.file]];
								}
							}
						}
				...
				if ([[NSFileManager defaultManager] fileExistsAtPath:vaultItemURL.path])
				{
					...
					[fileCoordinator coordinateWritingItemAtURL:vaultItemURL options:NSFileCoordinatorWritingForReplacing error:&fileCoordinatorError byAccessor:^(NSURL * _Nonnull newURL) {
						OCLogDebug(@"Coordinated write of %@ via %@", vaultItemURL, newURL);
						storeFile(newURL, YES);
					}];

					if (fileCoordinatorError != nil)
					{
						OCLogError(@"File coordination error %@ => manually updating file at %@", fileCoordinatorError, vaultItemURL);
						storeFile(vaultItemURL, YES);
					}
		...
		[resultItem removeSyncRecordID:syncContext.syncRecord.recordID activity:OCItemSyncActivityDownloading];
		...
		[syncContext completeWithError:downloadError core:self.core item:resultItem parameter:downloadedFile];
	...
	if (downloadError != nil)
	{
		...
		else
		{
			...
			if ([downloadError.domain isEqual:OCHTTPStatusErrorDomain] && (downloadError.code == OCHTTPStatusCodePRECONDITION_FAILED))
			{
				...
				OCLogError(@"Download %@ error %@ => ETag on the server likely changed from the last known ETag", resultItem, downloadError);

				// Request refresh of parent path
				if (resultItem.path.parentPath != nil)
				{
					syncContext.refreshPaths = @[ resultItem.path.parentPath ];
				}

				// For anything else: wait for metadata update to happen
				if ((_resolutionRetries < 3) && (resultItem.path != nil)) // limit retries until bringing up a user-facing error
				{
					_resolutionRetries++;
					...
					[syncContext transitionToState:OCSyncRecordStateReady withWaitConditions:@[
						[OCWaitConditionMetaDataRefresh waitForPath:resultItem.path versionOtherThan:resultItem.itemVersionIdentifier until:[NSDate dateWithTimeIntervalSinceNow:120.0]]
					]];
			...
			if (!handledError)
			{
				...
				OCLogError(@"Wrapping download %@ error %@ into cancellation issue", resultItem, downloadError);
