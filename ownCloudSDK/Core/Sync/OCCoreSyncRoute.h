...
typedef BOOL(^OCCoreSyncRouteAction)(OCCore *core, OCCoreSyncParameterSet *syncParameterSet);

@interface OCCoreSyncRoute : NSObject

@property(copy) OCCoreSyncRouteAction scheduler;	//!< Used to schedule network request(s) for an action. Return YES if scheduling worked. Return NO and possibly an error in OCCoreSyncParameterSet.error if not.
@property(copy) OCCoreSyncRouteAction resultHandler;	//!< Used to handle the result of an action (usually following receiving an OCEvent). Return YES if the action succeeded and the sync record has been made obsolete by it (=> can be removed). Return NO if the action has not yet completed or succeeded and add OCConnectionIssue(s) to OCCoreSyncParameterSet.issues where appropriate.
